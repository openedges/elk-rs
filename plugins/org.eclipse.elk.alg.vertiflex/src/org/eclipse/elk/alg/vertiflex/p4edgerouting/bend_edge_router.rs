...
impl BendEdgeRouter {
    ...
    fn route_edges(&self, node: &ElkNodeRef) {
        for edge in ElkGraphUtil::all_outgoing_edges(node) {
            ...
            let bend_height =
                node_get_property(&target, InternalProperties::EDGE_BEND_HEIGHT).unwrap_or(end_y);
            let epsilon = 0.0001;
            let parent_spacing = target
                .borrow()
                .parent()
                .and_then(|parent| node_get_property(&parent, CoreOptions::SPACING_NODE_NODE))
                .unwrap_or(0.0);
            if (bend_height - (end_y - parent_spacing / 2.0)).abs() > epsilon {
...
impl Default for BendEdgeRouter {
    fn default() -> Self {
        Self::new()
    }
}

impl ILayoutPhase<VertiFlexLayoutPhases, ElkNodeRef> for BendEdgeRouter {
    fn process(&mut self, graph: &mut ElkNodeRef, progress_monitor: &mut dyn IElkProgressMonitor) {
        progress_monitor.begin("BendEdgeRouter", 1.0);
...
fn create_bend_point(section: &ElkEdgeSectionRef, x: f64, y: f64) {
    ...
    section.borrow_mut().bend_points().push(bend_point);
}
