...
fn can_reverse_outgoing_edge(source_constraint: LayerConstraint, edge: &LEdgeRef) -> bool {
    ...
    if target_constraint == LayerConstraint::LastSeparate {
        ...
        let (source_node, source_side, target_side) = {
            let eg = edge.lock();
            let src = eg.source();
            let tgt = eg.target();
            let src_node = src.as_ref().and_then(|p| p.lock().node());
            let src_side = src.map(|p| p.lock().side()).unwrap_or(PortSide::Undefined);
            let tgt_side = tgt.map(|p| p.lock().side()).unwrap_or(PortSide::Undefined);
            (src_node, src_side, tgt_side)
        };
        let source_is_normal = source_node
            .as_ref()
            .map(|n| n.lock().node_type() == NodeType::Normal)
            .unwrap_or(false);
        let target_is_normal = target_node.lock().node_type() == NodeType::Normal;
...
fn can_reverse_incoming_edge(target_constraint: LayerConstraint, edge: &LEdgeRef) -> bool {
    ...
    if source_constraint == LayerConstraint::FirstSeparate {
        // Allow reversal if both source and target are NORMAL nodes with EAST→WEST direction
        let (target_node, source_side, target_side) = {
            let eg = edge.lock();
            let src = eg.source();
            let tgt = eg.target();
            let tgt_node = tgt.as_ref().and_then(|p| p.lock().node());
            let src_side = src.map(|p| p.lock().side()).unwrap_or(PortSide::Undefined);
            let tgt_side = tgt.map(|p| p.lock().side()).unwrap_or(PortSide::Undefined);
            (tgt_node, src_side, tgt_side)
        };
        let source_is_normal = source_node.lock().node_type() == NodeType::Normal;
