...
impl LGraphUtil {
    ...
    pub fn resize_node(node: &LNodeRef, new_size: &KVector, move_ports: bool, move_labels: bool) {
        let (old_size, ports, labels, fixed_ports) = match node.lock_ok() {
            Some(mut node_guard) => (
        ...
        for node in layerless_nodes {
            ...
            for port in ports {
                ...
                for edge in outgoing_edges {
                    let labels = if let Some(mut edge_guard) = edge.lock_ok() {
    ...
    pub fn place_nodes_horizontally(layer: &LayerRef, xoffset: f64) {
        let (nodes, layer_size) = if let Some(mut layer_guard) = layer.lock_ok() {
        ...
        for node in nodes {
            let (alignment, node_size, margin, ports) = if let Some(mut node_guard) = node.lock_ok() {
                (
                    node_guard
    ...
    pub fn find_max_non_dummy_node_width(
        ...
        let nodes = if let Some(layer_guard) = layer.lock_ok() {
    ...
    pub fn compute_graph_properties(layered_graph: &LGraphRef) {
        ...
        for node in nodes {
            ...
            for port in ports {
                ...
                for edge in outgoing_edges {
                    let labels = if let Some(edge_guard) = edge.lock_ok() {
        ...
        if let Some(mut graph_guard) = layered_graph.lock_ok() {
    ...
    pub fn calc_port_side(port: &LPortRef, direction: Direction) -> PortSide {
        let (node, pos, size) = if let Some(mut port_guard) = port.lock_ok() {
            (
                port_guard.node(),
        ...
        let node_size = if let Some(mut node_guard) = node.lock_ok() {
            *node_guard.shape().size_ref()
        } else {
            return PortSide::Undefined;
        };
    ...
    pub fn provide_collector_port(
        ...
        match port_type {
            PortType::Input => {
                ...
                if let Some(mut port) = created.lock_ok() {
    ...
    pub fn initialize_port(
        ...
        if port_side == PortSide::Undefined && port_constraints.is_side_fixed() {
            ...
            let should_set_border_offset = if let Some(mut port_guard) = port.lock_ok() {
        ...
        if port_constraints.is_ratio_fixed() {
            let ratio = if let Some(mut port_guard) = port.lock_ok() {
                let node_size = port_guard
                    .node()
                    .and_then(|node| node.lock_ok().map(|mut node| *node.shape().size_ref()))
    ...
    pub fn get_external_port_position(
        ...
        let (mut port_pos, port_offset, _, ext_side) =
            if let Some(mut dummy_guard) = port_dummy.lock_ok() {
    ...
    pub fn change_coord_system(point: &mut KVector, old_graph: &LGraphRef, new_graph: &LGraphRef) {
        ...
        loop {
            let (offset, padding, parent_node) = if let Some(graph_guard) = graph.lock_ok() {
                (
                    *graph_guard.offset_ref(),
                    graph_guard.padding_ref().clone(),
    ...
    pub fn get_individual_or_inherited<T: Clone + Send + Sync + 'static>(
        ...
        let (mut result, graph) = if let Some(mut node_guard) = node.lock_ok() {
    ...
    pub fn get_direction(graph: &LGraphRef) -> Direction {
        let (direction, aspect_ratio) = if let Some(mut graph_guard) = graph.lock_ok() {
            (
                graph_guard
