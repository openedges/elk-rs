...
use crate::org::eclipse::elk::alg::layered::graph::{
    index_of_arc, LEdge, LGraph, LGraphUtil, LNodeRef, LPortRef, NodeRefKey, NodeType,
};
use crate::org::eclipse::elk::alg::layered::options::{
    CenterEdgeLabelPlacementStrategy, InternalProperties, LayeredOptions, PortType,
};

pub static INCLUDE_LABEL: LazyLock<Property<bool>> = LazyLock::new(|| {
...
impl ILayoutProcessor<LGraph> for LabelDummySwitcher {
    fn process(&mut self, layered_graph: &mut LGraph, monitor: &mut dyn IElkProgressMonitor) {
        ...
        let mut label_dummy_infos =
            gather_label_dummy_infos(layered_graph, default_strategy);

        self.layer_widths = vec![0.0; layered_graph.layers().len()];

        for strategy in STRATEGIES {
            if strategy.uses_label_size_information() {
                if let Some(infos) = label_dummy_infos.get(&strategy) {
                    if !infos.is_empty() {
                        self.calculate_layer_widths(layered_graph);
                        break;
                    }
                }
            }
...
impl LabelDummySwitcher {
    fn calculate_layer_widths(&mut self, layered_graph: &LGraph) {
        for layer in layered_graph.layers() {
            let layer_id = layer
                .lock()
                .ok()
                ...
                .unwrap_or(0);
            if layer_id >= 0 {
                let width = LGraphUtil::find_max_non_dummy_node_width(layer, false);
    ...
    fn find_center_layer_target_id(&self, label_dummy_info: &LabelDummyInfo) -> usize {
        ...
        let threshold = layer_width_sums
            .last()
            .copied()
            .unwrap_or(0.0)
            / 2.0;
    ...
    fn compute_layer_width_sums(&self, label_dummy_info: &LabelDummyInfo) -> Vec<f64> {
        ...
        let min_space_between_layers = graph
            .lock()
            .ok()
            .map(|mut graph_guard| {
                let edge_node_spacing = graph_guard
                    .get_property(LayeredOptions::SPACING_EDGE_NODE_BETWEEN_LAYERS)
                    .unwrap_or(0.0)
                    * 2.0;
                let node_node_spacing = graph_guard
                    .get_property(LayeredOptions::SPACING_NODE_NODE_BETWEEN_LAYERS)
                    .unwrap_or(0.0);
                edge_node_spacing.max(node_node_spacing)
            })
            .unwrap_or(0.0);
        ...
        for left_dummy in &label_dummy_info.left_long_edge_dummies {
            ...
            current_sum += self.layer_widths.get(layer_id).copied().unwrap_or(0.0)
        ...
        current_sum += self.layer_widths.get(label_layer_id).copied().unwrap_or(0.0)
            + min_space_between_layers;
        if index < sums.len() {
            sums[index] = current_sum;
            index += 1;
    ...
    fn find_end_layer_target_id(&self, label_dummy_info: &LabelDummyInfo, head_layer: bool) -> usize {
    ...
    fn set_end_layer_node_alignment(&self, label_dummy_info: &LabelDummyInfo) {
        ...
        if let Ok(mut node_guard) = label_dummy_info.label_dummy.lock() {
    ...
    fn compute_space_efficient_assignment(&mut self, label_dummy_infos: &mut Vec<LabelDummyInfo>) {
        ...
        for label_index in 0..non_trivial_labels.len() {
            let target =
                self.find_potentially_widest_layer(&non_trivial_labels, label_index);
    ...
    fn assign_layer(&mut self, label_dummy_info: &LabelDummyInfo, target_layer_index: usize) {
        ...
        let new_layer_id = node_layer_id(&label_dummy_info.label_dummy)
            .unwrap_or(target_layer_index);
        let dummy_width = label_dummy_info.label_dummy_width();
    ...
    fn swap_nodes(&self, label_dummy: &LNodeRef, long_edge_dummy: &LNodeRef) {
        let (layer1, layer2) = match (
            label_dummy.lock().ok().and_then(|node_guard| node_guard.layer()),
            long_edge_dummy
                .lock()
                .ok()
        ...
        let (input1, output1) = match label_dummy.lock().ok() {
            Some(node_guard) => {
        ...
        let (input2, output2) = match long_edge_dummy.lock().ok() {
            Some(node_guard) => {
                let input = node_guard.ports_by_type(PortType::Input).first().cloned();
                let output = node_guard.ports_by_type(PortType::Output).first().cloned();
                (input, output)
        ...
        F: Fn(&LNodeRef) -> Option<LNodeRef>,
    {
        ...
        while node_type(&long_edge_dummy) == NodeType::LongEdge {
            ...
            if let Ok(mut node_guard) = long_edge_dummy.lock() {
                node_guard.set_property(InternalProperties::LONG_EDGE_BEFORE_LABEL_DUMMY, Some(value));
            }
...
fn port_incoming_edges(port: &LPortRef) -> Vec<crate::org::eclipse::elk::alg::layered::graph::LEdgeRef> {
    port.lock()
        .ok()
...
fn port_outgoing_edges(port: &LPortRef) -> Vec<crate::org::eclipse::elk::alg::layered::graph::LEdgeRef> {
    port.lock()
        .ok()
...
impl LabelDummyInfo {
    ...
    fn total_dummy_count(&self) -> usize {
        self.rightmost_layer_id.saturating_sub(self.leftmost_layer_id) + 1
    }
