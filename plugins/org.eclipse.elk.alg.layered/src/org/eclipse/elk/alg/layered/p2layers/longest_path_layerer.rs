...
impl LongestPathLayerer {
    ...
    fn visit(&mut self, node: &LNodeRef, graph: &mut LGraph, graph_ref: &LGraphRef) -> i32 {
        ...
        for port in ports {
            ...
            for edge in outgoing {
                ...
                let target_height = self.visit(&target_node, graph, graph_ref);
    ...
    fn put_node(
        ...
        let layer_index = graph.layers().len().saturating_sub(height);
...
impl Default for LongestPathLayerer {
    fn default() -> Self {
        Self::new()
    }
}

impl ILayoutPhase<LayeredPhases, LGraph> for LongestPathLayerer {
    fn process(&mut self, graph: &mut LGraph, monitor: &mut dyn IElkProgressMonitor) {
        monitor.begin("Longest path layering", 1.0);
