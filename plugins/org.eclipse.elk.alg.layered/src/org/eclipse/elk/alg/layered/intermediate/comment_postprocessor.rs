...
impl ILayoutProcessor<LGraph> for CommentPostprocessor {
    fn process(&mut self, layered_graph: &mut LGraph, monitor: &mut dyn IElkProgressMonitor) {
        ...
        for layer in layers {
            ...
            for node in nodes {
                let (top_boxes, bottom_boxes) = if let Some(mut node_guard) = node.lock_ok() {
    ...
    if let Some(top_boxes) = top_boxes {
        ...
        for box_node in top_boxes {
            ...
            if let Some(box_port) = get_box_port(box_node) {
                if let Some(mut box_port_guard) = box_port.lock_ok() {
            ...
            if let Some(node_port) = comment_conn_port(box_node) {
                ...
                if let Some(mut node_port_guard) = node_port.lock_ok() {
...
fn comment_conn_port(comment_box: &LNodeRef) -> Option<LPortRef> {
    comment_box.lock_ok().and_then(|mut box_guard| {
        if box_guard
            .shape()
            .graph_element()
            .properties()
