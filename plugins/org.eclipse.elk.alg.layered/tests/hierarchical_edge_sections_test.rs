...
fn assert_edge_has_sections(edge: &org_eclipse_elk_graph::org::eclipse::elk::graph::ElkEdgeRef, label: &str) {
...
fn hierarchy_center_edge_label_problem_writes_sections() {
    ...
    let graph =
        load_layered_graph_from_elkt(&path).expect("hierarchy_center_edge_label_problem should load");
...
fn include_children_fixed_order_writes_sections() {
    ...
    let edge = find_edge_by_identifier(&graph, "n2", "n1")
        .expect("edge n2 -> n1 should exist");
