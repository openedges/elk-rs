...
fn set_in_layer_order_constraint(this_node: &LNodeRef, before_node: &LNodeRef) {
    if let Some(mut node_guard) = this_node.lock_ok() {
        node_guard.set_property(
...
fn nodes_in_layer(graph: &LGraphRef, layer_idx: usize) -> Vec<LNodeRef> {
    let graph_guard = graph.lock();    let layer = graph_guard.layers().get(layer_idx).cloned().expect("layer");
...
fn run_greedy_switcher(graph: &LGraphRef, greedy_type: CrossMinType) {
    ...
    let mut graph_guard = graph.lock();    minimizer.process(&mut graph_guard, &mut monitor);
}
...
fn graph_nodes_in_different_layout_units_prevent_switch() -> LGraphRef {
    ...
    if let Some(mut node_guard) = right_nodes[1].lock_ok() {
        node_guard.set_property(
            InternalProperties::IN_LAYER_LAYOUT_UNIT,
            Some(right_nodes[2].clone()),
        );
    }
    if let Some(mut node_guard) = right_nodes[2].lock_ok() {
        node_guard.set_property(
            InternalProperties::IN_LAYER_LAYOUT_UNIT,
            Some(right_nodes[2].clone()),
        );
...
fn self_loops() {
    for_each_greedy_type(|greedy_type| {
        ...
        for layer in graph.lock().layers().clone() {
            for node in layer.lock().nodes().clone() {
