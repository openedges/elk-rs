...
#[test]
fn comment_node_margin_calculator_extends_margins_for_comments() {
    ...
    assert!((node_guard.margin().left - 1.0).abs() < TOLERANCE);
    assert!((node_guard.margin().right - 1.0).abs() < TOLERANCE);
}

#[test]
fn comment_node_margin_calculator_leaves_node_without_comments_unchanged() {
    let (graph, layer) = new_graph_with_layer();
