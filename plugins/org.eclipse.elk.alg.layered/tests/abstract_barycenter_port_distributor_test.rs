...
fn port_ptrs(node: &LNodeRef) -> Vec<usize> {
    ...
    ports.iter().map(|port| std::sync::Arc::as_ptr(port) as usize).collect()
}
...
fn distribute_ports_in_complete_graph(graph: &LGraphRef) {
    ...
    for i in 0..node_order.len() {
        info.port_distributor().distribute_ports_while_sweeping(&node_order, i, true);
    }
    for i in (0..node_order.len()).rev() {
        info.port_distributor().distribute_ports_while_sweeping(&node_order, i, false);
    }
}
