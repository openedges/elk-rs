...
- 단계 진행률(다음 작업 체크리스트 기준): 100.0% (완료 19/19, 미완료 0) [2026-02-17 갱신]
...
- [x] Step L1-9: `next_to_port_if_possible_inside` (5 diffs) — port x +20
...
- [x] Step L3-2: compound node width calculation Java 비교/정합
  - 완료(2026-02-16): nested width/height drift 상위 모델 전수 대조(155/65)와 `701_portLabels` 단건 정밀 비교(`diffs=259`, `width/height=68`)로 root cause를 `alg.common nodespacing` 포트/라벨 계산 phase 미포팅으로 확정. 즉시 정합 방법(Java `PortContext`/`Horizontal|VerticalPortPlacementSizeCalculator`/`PortPlacementCalculator`/`PortLabelPlacementCalculator`/`NodeSizeCalculator` 포팅)과 TODO를 `perf/model_parity_full/l3_2_compound_width_analysis.md`에 정리
- [x] Step L3-3: crossing minimization 결과 비교 (Phase 3 entry point)
  - 완료(2026-02-16): ordering mismatch 30모델/51레이어를 `l3_3_ordering_diff.tsv`로 고정하고, 전략 매핑(`INTERACTIVE=11`, `default=19`) 및 trace 비교를 통해 `Interactive` 경로의 phase3 미실행을 확인. Java 대비 root cause를 `CrossingMinimizationStrategy::Interactive -> NoOpPhase` 미구현으로 확정, 포팅/wiring/회귀 테스트 계획을 `l3_3_crossing_minimization_analysis.md`에 문서화
- [x] Step L3-4: parity 재실행 및 대규모 진행률 갱신
  - 완료(2026-02-16): full parity 재실행 결과 `matches=1106`, `drift=333`, `total_diffs=6269`, `errors/timeouts=0`, `java_non_ok=9`로 기준선 유지. drift 재분류(`drift_summary_l3_4.txt`)도 `other=165`, `layering_diff=138`, `ordering_diff=30` 동일 확인
- [x] Step L3-5: Java `InteractiveCrossingMinimizer` 포팅 및 strategy wiring
  - 완료(2026-02-17): `p3order/interactive_crossing_minimizer.rs` 신규 추가, `CrossingMinimizationStrategy::Interactive`를 실 구현으로 연결, `p3order/mod.rs` export 반영. 교착 원인(`interactive_reference_point()`의 graph 재락)을 제거하고 position 계산을 phase-local option 기반으로 보정. 회귀 테스트 `interactive_crossing_minimizer_test` 3건 추가/통과, `issue_515_test`/`basic_crossing_minimization_test` 통과 확인
