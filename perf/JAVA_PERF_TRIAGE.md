...
This guide is for failures in `.github/workflows/perf.yml` Java steps.
...
- Java execution runs with external ELK isolation (`JAVA_PERF_EXTERNAL_ISOLATE=true`).
...
Typical symptom:
- `run_perf_and_compare_java.sh` fails before compare report generation.

Check in order:
1. DNS preflight (`repo.eclipse.org`, `repo.maven.apache.org`) result in logs.
2. Maven/Tycho repo isolation path (`JAVA_PERF_MVN_LOCAL_REPO`) value.
...
### 2) Java parity gate failed (`perf/java_vs_rust.md`)

Typical symptom:
- `check_java_perf_parity.sh` fails on results compare.

Check in order:
1. Rust perf input freshness (`perf/results_layered_issue_scenarios.csv`).
2. Java CSV freshness (`perf/java_results_layered_issue_scenarios.csv`).
...
### 3) Baseline parity gate failed (`perf/java_vs_rust_baseline.md`)

Typical symptom:
- `check_java_perf_parity.sh` fails in baseline mode.
...
Typical symptom:
- `perf/java_baseline_candidate_check.md` status is `not_ready` or step failed.
...
- `perf/java_vs_rust.md` exists and parity passed.
- `perf/java_vs_rust_baseline.md` exists and parity passed.
- `perf/java_baseline_candidate_check.md` is `ready`.
