...
- **Phase 4-A** (Property typed wrapper): `get_property`가 `&mut self` 요구하므로 typed accessor도 동일 — 보일러플레이트 감소 미미. `get_property`를 `&self`로 변경 후 재검토
- **Phase 3-A** (Arc::ptr_eq 검색 개선): `remove_arc`/`index_of_arc`는 그래프 구조 변경 시에만 호출, 핫루프는 arena/CSR 사용 — 실제 성능 영향 낮아 보류
- **Phase 3-B** (불필요한 clone 제거): 338건 Vec clone 분석 — 대부분 `Arc<Mutex>` 아키텍처에서 데드락 방지를 위해 구조적 필수. 근본 해결은 Phase 5-A로만 가능
- **Phase 5-A** (Force/Stress owned struct): 894줄 그래프 + 95 lock, arena/index 재설계 필요. SoA가 핫루프 커버하므로 성능 이득 미미 — 별도 전용 리팩토링으로 진행
- **Phase 5-B** (ElkGraph arena 전환): import/export 전체 경로 변경, 높은 위험 — 별도 프로젝트로 분리
