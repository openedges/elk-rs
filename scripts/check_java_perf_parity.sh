...
if [ ! -f "$RUST_FILE" ]; then
  echo "missing rust perf file: $RUST_FILE" >&2
  exit 1
fi
if [ ! -f "$JAVA_FILE" ]; then
  echo "missing java perf file: $JAVA_FILE" >&2
  exit 1
fi
...
for scenario in $(cat "$tmp_scenarios"); do
  ...
  avg_fail=$(awk -v v="$avg_regress" -v th="$THRESHOLD" 'BEGIN { if (v > th) { print 1 } else { print 0 } }')
  ops_fail=$(awk -v v="$ops_regress" -v th="$THRESHOLD" 'BEGIN { if (v > th) { print 1 } else { print 0 } }')

  if [ "$avg_fail" -eq 1 ]; then
    echo "$scenario: avg_ms regression vs java ${avg_regress}% (> ${THRESHOLD}%)"
    fail=1
  fi
  if [ "$ops_fail" -eq 1 ]; then
    echo "$scenario: scenarios_per_sec regression vs java ${ops_regress}% (> ${THRESHOLD}%)"
    fail=1
  fi
  if [ "$avg_fail" -eq 0 ] && [ "$ops_fail" -eq 0 ]; then
    echo "$scenario: ok vs java (avg_ms Δ${avg_regress}%, scenarios_per_sec Δ${ops_regress}%)"
  fi
done
