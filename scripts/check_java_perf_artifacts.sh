...
REPORT_FILE=${2:-perf/java_vs_rust.md}
...
if [ "$JAVA_GENERATE_ENABLED" = "true" ] && [ "$JAVA_GENERATE_DRY_RUN" != "true" ]; then
  if [ ! -s "$JAVA_FILE" ]; then
    echo "missing or empty generated Java perf CSV: $JAVA_FILE" >&2
    exit 1
  fi
fi
...
if [ -s "$JAVA_FILE" ]; then
  ...
  if [ "$row_count" -lt "$effective_min_rows" ]; then
    echo "java perf CSV data row count too small: rows=$row_count min=$effective_min_rows configured_min=$JAVA_ARTIFACT_MIN_ROWS required_scenarios=$required_scenario_count file=$JAVA_FILE" >&2
    exit 1
  fi
  ...
  if [ -n "$JAVA_ARTIFACT_REQUIRED_SCENARIOS" ]; then
    ...
    for required in "$@"; do
      ...
      if ! grep -Fxq "$required_trimmed" "$tmp_scenarios"; then
        echo "java perf CSV missing required scenario '$required_trimmed': $JAVA_FILE" >&2
        exit 1
      fi
    done
  fi
...
echo "java perf artifacts verified: report=$REPORT_FILE java_csv=$JAVA_FILE"
